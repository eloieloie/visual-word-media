...
- Keep the graph current after code edits. A local opencode plugin (`.opencode/plugins/graphify-refresh.ts`) auto-runs `graphify update .` (debounced 3s) after each `edit`/`write` tool call — no action needed when it is loaded. If the plugin is not active (e.g. fresh session not yet restarted, or running outside opencode), run `graphify update .` manually before querying the graph. The update is AST-only, no API cost.
